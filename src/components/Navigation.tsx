...
import { Menu, X, ChevronDown, Leaf, Home, BookOpen, Newspaper, Users, Mail, Sprout } from 'lucide-react';
import { navigationConfig } from '../lib/site-config';

const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  Home, BookOpen, Newspaper, Users, Mail, Sprout, Leaf, Menu, X, ChevronDown,
};
...
export function Navigation() {
  ...
  // Smart link handler: page routes (/) navigate, hash anchors (#) scroll
  ...
  // Legacy alias kept for ContactForm / OurStory internal use
  const scrollToSection = handleNavLink;
      ...
      <div className="container-custom flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8" role="menubar">
          {navLinks.map((link) => (
            ...
            >
              <button
                onClick={() => !link.dropdown && handleNavLink(link.href)}
              ...
              >
                {link.name}
                {link.dropdown && (
                  <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${activeDropdown === link.name ? 'rotate-180' : ''}`} aria-hidden="true" />
                )}
              </button>
      ...
      >
        <div className="container-custom py-8 flex flex-col gap-2">
          {navLinks.map((link, index) => {
            ...
            return (
              <div key={link.name} className="animate-fade-up" style={{ animationDelay: `${index * 0.1}s` }}>
                {link.dropdown ? (
                  <div>
                    <button
                    ...
                    >
                      ...
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${activeDropdown === link.name ? 'rotate-180' : ''}`} aria-hidden="true" />
                    </button>
                    <div className={`overflow-hidden transition-all duration-500 ${activeDropdown === link.name ? 'max-h-40' : 'max-h-0'}`} role="menu">
                      {link.dropdown.map((item) => (
                        <button
                          key={item.name}
                          onClick={() => scrollToSection(item.href)}
