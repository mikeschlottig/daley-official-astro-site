...
import { gardenCarouselConfig, navigationConfig } from '../lib/site-config';

export function GardenCarousel() {
  if (!gardenCarouselConfig.mainTitle || gardenCarouselConfig.slides.length === 0) return null;

  const slides = gardenCarouselConfig.slides;
    ...
    >
      ...
      <div className="container-custom relative">
        ...
        <div className="fade-up text-center mb-12">
          <span className="eyebrow">{gardenCarouselConfig.scriptText}</span>
          <span className="text-gold-500 text-xs uppercase tracking-[0.2em] mb-4 block">
            {gardenCarouselConfig.subtitle}
          </span>
          <h2 className="font-serif text-h1 text-white">{gardenCarouselConfig.mainTitle}</h2>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-0 items-stretch">
                ...
                {slides.map((_, index) => (
                  <button
                    ...
                    className={`h-1 rounded-full transition-all duration-300 ${
                      index === currentSlide ? 'w-8 bg-gold-500' : 'w-4 bg-white/40 hover:bg-white/60'
                    }`}
            ...
            <div className="lg:bg-white/5 lg:border-y lg:border-r lg:border-white/10 lg:rounded-r-lg p-8 lg:p-12 flex flex-col justify-center relative overflow-hidden">
              {slides.map((slide, index) => (
                ...
                >
                  {gardenCarouselConfig.locationTag && (
                    <div className="flex items-center gap-2 text-gold-500 text-sm mb-4">
                      <MapPin className="w-4 h-4" />
                      <span>{gardenCarouselConfig.locationTag}</span>
                    </div>
                  )}

                  <h3 className="font-serif text-h3 text-white mb-2">{slide.title}</h3>
                  <p className="text-white/70 text-lg mb-6">{slide.subtitle}</p>

                  <div className="flex items-baseline gap-2 mb-6">
                    <span className="font-serif text-5xl lg:text-6xl text-gold-500">{slide.area}</span>
                    <span className="text-white/70 text-lg">{slide.unit}</span>
                  </div>

                  <p className="text-white/75 leading-relaxed mb-8">{slide.description}</p>
        ...
        <div className="fade-up mt-8 flex justify-center lg:justify-start" style={{ transitionDelay: '0.2s' }}>
          <div className="flex items-center gap-4 text-sm">
            <span className="font-serif text-2xl text-gold-500">{String(currentSlide + 1).padStart(2, '0')}</span>
